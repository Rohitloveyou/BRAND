...
def logo(n):
	...
	return str(f"""d8888b.  .d8b.  db   dD db    db 🔥

88  `8D d8' `8b 88 ,8P' 88    88 

88   88 88ooo88 88,8P   88    88 

88   88 88~~~88 88`8b   88    88 

88  .8D 88   88 88 `88. 88b  d88 

Y8888D' YP   YP YP   YD ~Y8888P' 🔥

╔══════════════════════════════════════════════════╗

║ Creator  : TEAM DAKU                             ║

║ Github   : https://github.com/DAKU-MAKER         ║

║ Facebook : Sachin(Doremon)🙂❤️                    ║

║ Brothers :(Mujtaba Khan)(Krishna)(Rana Nadeem)   ║
...
def menu(n,t,c):
	...
	print(f" [{P}01{P}] Public Crack")

	print(f" [{P}02{P}] File Clone ")

	print(f" [{P}03{P}] Email Clone")

	print(f" [{P}04{P}] Remove cookie")
...
def crack_file():
	...
	file = input(f' [{hh}<{P}] Entrt File Name\n File : ')
	...
	try:
		...
		for data in uuid:
			...
			print('\r Collecting  %s id'%(len(dump)),end=" ")
...
def atur_atur():
	...
	ch = input(f' [{hh}1{P}] Oldest [{hh}2{P}] New [{hh}3{P}] Random\n └─ urutan : ')
	...
	cp = input(f' [{hh}!{P}] session options display[ya/no]\n └─ pilih  : ')
	...
	apk = input(f' [{hh}!{P}] app info display [ya/no]\n └─ pilih  : ')
	...
	ch = input(f' [{hh}1{P}] first last  [{hh}2{P}] first123 .etc  [{hh}3{P}] auto\n └─ sandi  : ')
...
def gabung():
	...
	exit(f'\r [{hh}<{P}] Crack Has Been Completed Total OK:{ok} Amount CP:{cp} ')
...
"Mozilla/5.0 (Linux; NetCast; U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3440.106 Safari/537.36 SmartTV/9.0 Crow/1.0"])
	...
	print(f"\r [{P}!{P}] {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
